...
function playerMove(board,pixel){
	if (gameEnd === false){
		...
		if (board[y][x] === 0){
			turn = turns%2+1
			board[y][x] = turn;
			const elem = document.getElementById([y,x].join());
		...
		if (winningPath.length > 0){
			gameEnd = true;
			const winner = (turns+1)%2;
			const myBorder = document.getElementById("container");
			...
			for (let path of winningPath[0]){
				let myPix = document.getElementById(path[0]+','+path[1]);
